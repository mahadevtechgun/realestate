...
const LocationStats = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const saleProperties = data.data?.filter((item) =>
          item.property_type?.includes("For Sale")
        ) || [];
  ...
  const icons = [FaCity, FaBuilding, FaWater, FaSun];
      ...
      <div className="max-w-[1300px] mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-[#A0A0A0] mt-4 max-w-2xl mx-auto animate-fade-up-delay-1 text-lg">
            Discover how many Properties are available for sale in prime locations
          </p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">

          {locationStats.map((items, index) => {

            const Icon = icons[index % icons.length]; // icons ko baari-baari se cycle karo

            return (
              <div
                key={index}
                className="group bg-gradient-to-br from-[#171B22] to-[#1a1a2e] rounded-2xl p-6 text-center border border-white/10 hover:border-[#D4AF37]/50 transition-all duration-500 shadow-[0_10px_30px_rgba(212,175,55,0.08)] hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(212,175,55,0.2)] animate-fade-up-delay-1"
              >
                <div className="w-16 h-16 rounded-2xl bg-[#D4AF37]/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-[#D4AF37] group-hover:scale-110 transition-all duration-500">
                  <Icon className="text-2xl text-[#D4AF37] group-hover:text-[#0F1115] transition-colors" />
                </div>
                ...
                <p className="text-[#A0A0A0] text-sm mt-1">Properties for Sale</p>
