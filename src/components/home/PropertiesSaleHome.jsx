...
const PropertiesSaleHome = ({ setIsLoginOpen, showViewAll = true }) => {
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProperties?.slice(0, 6).map((items, index) => (
                <div
                  key={index}
                  className="group bg-[#171B22] rounded-2xl overflow-hidden border border-white/10 hover:border-[#D4AF37]/50 hover:-translate-y-2 transition-all duration-500"
                >
                  <div className="relative h-[280px] overflow-hidden">
                    <WishlistIcon property={items} setIsLoginOpen={setIsLoginOpen} />
                    <Gallery items={items} />

                    <div className="absolute top-4 left-4 bg-[#D4AF37] px-4 py-1.5 rounded-full text-xs font-bold text-[#0F1115] uppercase z-10">
                      {items?.property_type}
                    </div>
            ...
            {filteredProperties.length === 0 && (
              <div className="text-center py-16">
                <h3 className="text-2xl font-bold text-white mb-2">No Properties Found for Sale</h3>
                <p className="text-[#A0A0A0]">There are currently no properties available for sale.</p>
              </div>
            )}
...
export default PropertiesSaleHome;